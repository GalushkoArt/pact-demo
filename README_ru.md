...
### 6. Проверить совместимость развертывания

Проверьте, можно ли безопасно развернуть поставщика:

```bash
make canideploy-all
```

Эта команда проверяет, могут ли price-service-provider-price и price-service-provider-orderbook быть развернуты без нарушения работы какого-либо потребителя.
...
Это пройдет без проверки новых контрактов, потому что результаты тестов кэшируются.

### 11. Принудительно запустить тесты поставщика

Принудительно запустите тесты поставщика, сначала очистив результаты тестов:
...
### 12. Проверить Pact Broker еще раз
...
### 13. Проверить совместимость развертывания снова
...
Теперь это должно пройти с `Computer says yes \o/` для обоих поставщиков.
